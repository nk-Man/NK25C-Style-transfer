...
if __name__=='__main__':
    ...
    model  = StyleTransferNet(decoder_ckpt='./decoder222.pth', device=device)
    model.eval()
    ...
    writer = SummaryWriter(log_dir='logs/ablation')
    writer.add_image("原始内容图",    orig_tensor.cpu()[0], 0)
    writer.add_image("风格迁移后",    x.cpu()[0],              0)
    writer.close()
